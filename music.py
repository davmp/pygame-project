...
class Music:
    def __init__(self):
        ...
        self.menu_track = pygame.mixer.Sound(os.path.join('assets', 'sounds', 'music', 'ambience_3.mp3'))
        self.base_track = pygame.mixer.Sound(os.path.join('assets', 'sounds', 'music', 'soft_layer.ogg'))
    ...
    def control_music(self):
        if config.music_volume > 0:
            if [x for x in config.npc_list if x.state == 'attacking' and not x.dead] and not config.paused or \
            ...
            if config.paused:
                if self.menu_volume < self.settings_volume * 0.2:
                    ...
                    pygame.mixer.Sound.set_volume(self.base_track, max(0, (
                                self.settings_volume - self.menu_volume) * config.music_volume))
                    pygame.mixer.Sound.set_volume(self.hard_track,
                                                  max(0, (self.hard_volume - self.menu_volume) * config.music_volume))
            else:
