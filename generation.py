...
class Generator:
    def __init__(self):
        ...
        self.max_npc_amount = config.current_level + 1
        self.max_npcs_per_segment = 3
        self.min_npcs_per_level = 3
        self.npc_spawn_chance = 20 + config.current_level * 1.5
    ...
    def generate_level(self, size, *seed):
        ...
        if not self.all_segs:
            for seg in config.segments_list:
        ...
        for row in range(len(array)):
            ...
            for col in range(len(array[row])):
                if not array[row][col]:
                    array[row][col] = Segment(
                        {'id': None, 'items': [], 'npcs': [], 'array': empty, 'doors': [], 'type': 'empty'})
    ...
    def rotate_segment(segment):
        ...
        for i in range(len(rotseg.array)):
            ...
            for tile in rotseg.array[i]:
                if config.texture_type[tile] == 'hdoor' or config.texture_type[tile] == 'vdoor':

                    for y in range(len(Texture.all_textures)):
                        if y != tile and os.path.samefile(Texture.all_textures[y], Texture.all_textures[tile]):
                            rotseg.array[i][t] = y
    ...
    def place_random_items(self):
        items = 0
        seed = config.current_level + self.seed
        ...
        for i in range(len(self.segpath)):
            ...
            for y in range(self.max_items_per_segment):
                random.seed(seed)
                if (len(seg.doors) <= 1 and self.spawn_chance_high >= random.randint(0, 100)) or (
                        len(seg.doors) > 1 and self.spawn_chance >= random.randint(0, 100)):
                    is_good = False
                    while not is_good:
                        ...
                        if not config.tile_solid[seg.array[randomy][randomx]] and not occupied:
                            ...
                            if config.item_types[item]['type'] in ['primary', 'secondary']:
                                ammo = [x for x in config.item_types if
                                        x['type'] == config.item_types[item]['effect'].ammo_type][0]
                                adjacents = [[randomx + 1, randomy], [randomx, randomy + 1], [randomx - 1, randomy],
                                             [randomx, randomy - 1]]
                                for pos in adjacents:
                                    occupied = [x for x in seg.items if x[0] == (
                                        max(0, min(pos[0], len(seg.array) - 1)),
                                        max(0, min(pos[1], len(seg.array) - 1)))]
                                    if self.ammo_spawn_chance >= random.randint(0, 100) and not config.tile_solid[
                                        self.segpath[i].array[max(0, min(pos[0], len(seg.array) - 1))][
                                            max(0, min(pos[1], len(seg.array) - 1))]] and not occupied:
    ...
    def spawn_random_npcs(self):
        ...
        while npc_amount < self.min_npcs_per_level:
            ...
            if not config.tile_solid[segment.array[randomy][randomx]] and not occupied:
                npc = random.choice(self.npc_probability)
